package net.ayld.diet.bundle.impl;
...
import net.ayld.diet.bundle.JarMaker;
...
public class TestManualJarMaker {
	...
	private static final Set<String> CORRECT_ZIPPED_ENTRY_NAMES = ImmutableSet.of(
			...
			Joiner.on(File.separator).join("net", "ayld", "diet") + File.separator,
			Joiner.on(File.separator).join("net", "ayld", "diet", "model") + File.separator,
			Joiner.on(File.separator).join("net", "ayld", "diet", "model", "ClassName.class"),
			"org" + File.separator,
